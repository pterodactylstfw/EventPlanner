...
int main() {
    ...
    while (true) {
        ...
        switch (optiune) {
            ...
            case 2: {
                unsigned int ID;
                std::cout<<"Introduceti ID-ul evenimentului dorit a fi sters: ";std::cin>>ID;
                if (planner.findEventByID(ID)!=nullptr)
                    planner.deleteEvent(ID);
                break;
            }

            case 3: { // Editare eveniment
                unsigned int ID;
                ...
                std::string titlu, descriere, data, ora, locatie;
                std::cout << "Titlu nou: "; std::getline(std::cin, titlu);
                std::cout << "Descriere noua: "; std::getline(std::cin, descriere);
                std::cout << "Data noua (Format: ZZ/LL/AAAA): "; std::cin >> data;
                std::cin.ignore();
                std::cout << "Ora noua (Format: hh:mm): "; std::cin >> ora;
                std::cin.ignore();
                std::cout << "Locatie noua: "; std::cin >> locatie;
                std::cin.ignore();
            ...
            case 5: { // Cautare eveniment dupa ID
                unsigned int ID;
            ...
            case 6: { // Cautare evenimente dupa titlu
                std::string titlu;
            ...
            case 7: { // Notifica despre urmatoarele evenimente
                planner.notifyUpcomingEvents();
                break;
            }

            case 8: { // Salveaza evenimentele in fisier
                std::string filename;
            ...
            case 9: { // Incarca evenimentele din fisier
                std::string filename;
